...
function sourceId(url: string) {
  return `src-${hashText(url.toLowerCase())}`;
}

function findingId(title: string) {
  return `finding-${hashText(normalized(title))}`;
}

function questionId(question: string) {
  return `question-${hashText(normalized(question))}`;
}

function uncertaintyId(label: string) {
  return `uncertainty-${hashText(normalized(label))}`;
}
...
function sourceFingerprint(runs: StoredRun[]) {
  const material = runs.map((run) => ({
    ...
    sources: run.sources.map((source) => source.url).sort(),
  ...
  for (const run of runs) {
    for (const source of run.sources) {
      ...
      sources.set(id, {
        ...
        firstSeenAt: current?.firstSeenAt ?? run.recordedAt,
        lastSeenAt: run.recordedAt,
      });
    }
  }
...
function compileFindings(runs: StoredRun[]) {
  ...
  for (const run of runs) {
    const points = run.brief?.executiveSummary?.points?.length
      ? run.brief.executiveSummary.points
      : run.brief?.highlights ?? [];
    for (const point of points) {
      ...
      findings.set(id, {
        ...
        title: point.title,
        detail: point.detail,
        confidence: confidenceForRun(run, run.brief?.citations?.length ?? 0),
        status: "current",
        sourceIds: [...new Set([...(previous?.sourceIds ?? []), ...ids])].slice(0, 8),
        firstSeenAt: previous?.firstSeenAt ?? run.recordedAt,
...
function compileQuestions(runs: StoredRun[], sourceRefs: SourceReference[]) {
  ...
  for (const run of runs) {
    for (const question of run.brief?.nextQuestions ?? []) {
      ...
      questions.set(id, {
        ...
        sourceIds: previous?.sourceIds ?? fallbackSourceIds,
        firstSeenAt: previous?.firstSeenAt ?? run.recordedAt,
      });
    }
  }
...
function compileUncertainties(runs: StoredRun[], sourceRefs: SourceReference[]) {
  ...
  const fallbackSourceIds = sourceRefs.slice(-3).map((source) => source.id);
  for (const run of runs) {
    for (const hypothesis of run.brief?.hypotheses ?? []) {
...
export function compileLivingDocument(garden: Garden, previous: LivingDocument | null = null, now = new Date().toISOString()): LivingDocument {
  ...
  const hasChangedInput = !previous || previous.basedOn.sourceFingerprint !== fingerprint;
  const documentVersion = previous ? previous.documentVersion + (hasChangedInput ? 1 : 0) : 1;
  const executiveSummary = latest?.brief?.executiveSummary?.takeaway
    ?? latest?.brief?.summary
    ?? (runs.length ? `Research is accumulating across ${runs.length} growth cycles.` : "This garden is waiting for its first research run.");
    ...
    sections: [
      {
        ...
        summary: latest?.brief?.summary ?? executiveSummary,
        findingIds: findings.filter((finding) => finding.status === "current").map((finding) => finding.id),
      },
      {
        id: "evidence-base",
        title: "Evidence base",
        summary: sourceRefs.length ? `${sourceRefs.length} distinct sources currently support this document.` : "No source evidence has been recorded yet.",
  ...
  current.changes = hasChangedInput ? diffLivingDocuments(previous, current, latest?.recordedAt ?? now) : [];
  if (!hasChangedInput && previous) {
