...
function deterministicChangeId(operation: DocumentPatchOperation, version: number, label: string, index: number) {
  const material = `${version}|${operation.op}|${label}|${operation.recordedAt}|${index}|${operation.reason}`;
  let hash = 2166136261;
...
function changeFor(operation: DocumentPatchOperation, version: number, label: string, detail: string, index: number): DocumentChange {
  return {
    id: deterministicChangeId(operation, version, label, index),
...
export function applyDocumentPatch(document: LivingDocument, patch: DocumentPatch): LivingDocument {
  ...
  patch.operations.forEach((operation, index) => {
    ...
    if (operation.op === "add_finding") {
      ...
      emitted.push(changeFor(operation, patch.toVersion, operation.finding.title, operation.reason, index));
      return;
    }
    ...
    if (operation.op === "add_open_question") {
      ...
      emitted.push(changeFor(operation, patch.toVersion, operation.question.question, operation.reason, index));
      return;
    }
    ...
    if (operation.op === "update_section_summary") {
      ...
      emitted.push(changeFor(operation, patch.toVersion, section.title, operation.reason, index));
      return;
    }
    ...
    if (operation.op === "resolve_open_question") {
      ...
      emitted.push(changeFor(operation, patch.toVersion, question.question, operation.reason, index));
      return;
    }
    ...
    if (operation.op === "update_finding") {
      ...
      emitted.push(changeFor(operation, patch.toVersion, finding.title, operation.reason, index));
