...
export default async function PublicGardenPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="public-garden">
    ...
    {garden.latestRun ? <>
      <section className="research-brief"><p className="eyebrow">LATEST GROWTH · {garden.latestRun.query}</p>{garden.latestRun.brief && <><h2>{garden.latestRun.brief.headline}</h2><p>{garden.latestRun.brief.summary}</p></>}</section>
